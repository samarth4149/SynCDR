...
def setup_logging(log_root, pre_fix=''):
    import datetime
    import logging
    log_fn = os.path.join(log_root, pre_fix+"LOG.{0}.txt".format(datetime.date.today().strftime("%y%m%d")))
    logging.basicConfig(filename=log_fn, filemode='w', level=logging.DEBUG, format='%(message)s')
    # also log into console
    console = logging.StreamHandler()
    # console.setLevel(logging.INFO)
    logging.getLogger('').addHandler(console)
    logging.getLogger().setLevel(logging.INFO)
    print('Loging into %s...' % log_root)
